...
(defun manage-waste-letter (infos errors)
  (with-standard-html-frame (stream
    ...
    (let ((html-template:*string-modifier* #'escape-string-all-but-double-quotes)
          ...
          (json-labs        (array-autocomplete-laboratory (get-session-user-id)))
          (json-labs-id     (array-autocomplete-laboratory-id (get-session-user-id)))
...
(defun validate-letter-request (username lab-id building-id weight cer-id
  ...
  (let* ((all-adrs   (%get-all-objects-from-dirty-ids 'db:adr-code adrs))
    ...
    (when (not (user-lab-associed-p (get-session-user-id)
                                    lab-id))
      (push (_ "You are not in charge for this laboratory") errors))
    errors))
