...
(define-lab-route search-chem-prod ("/search-chem-prod/" :method :get)
  (with-authentication
    (if (or (get-clean-parameter +search-chem-id+)
            (get-clean-parameter +search-chem-owner+)
            (get-clean-parameter +search-chem-name+)
            (get-clean-parameter +search-chem-building+)
            (get-clean-parameter +search-chem-floor+)
            (get-clean-parameter +search-chem-storage+)
            (get-clean-parameter +search-chem-shelf+))
        (search-products (get-clean-parameter +search-chem-id+)
                         (get-clean-parameter +search-chem-owner+)
                         (get-clean-parameter +search-chem-name+)
                         (get-clean-parameter +search-chem-building+)
                         (get-clean-parameter +search-chem-floor+)
...
(define-lab-route post-search-chem-prod ("/post-search-chem-prod/" :method :post)
  (with-authentication
    (let ((errors (if (and (get-post-filename +name-chem-struct-data+)
                      ...
                      nil)))
      (if (and (not errors)
               (or (post-clean-parameter     +search-chem-id+)
                   (post-clean-parameter     +search-chem-owner+)
                   (post-clean-parameter     +search-chem-name+)
                   (post-clean-parameter     +search-chem-building+)
                   (post-clean-parameter     +search-chem-floor+)
                   (post-clean-parameter     +search-chem-storage+)
                   (post-clean-parameter     +search-chem-shelf+)
                   (get-post-filename +name-chem-struct-data+)))
          (search-products (post-clean-parameter +search-chem-id+)
                           (post-clean-parameter +search-chem-owner+)
                           (post-clean-parameter +search-chem-name+)
                           (post-clean-parameter +search-chem-building+)
                           (post-clean-parameter +search-chem-floor+)
          ...
          (let ((errors (loop named add-loop repeat (parse-integer (get-clean-parameter +name-count+)) do
                             (let ((actual-notes (regex-replace-all "\\n"
                               ...
                               (multiple-value-bind (err success)
                                   (add-single-chem-prod (get-clean-parameter +name-chem-id+)
                                                         (get-clean-parameter +name-chp-storage-id+)
                                                         (get-clean-parameter +name-shelf+)
                                                         (get-clean-parameter +name-quantity+)
                                                         (get-clean-parameter +name-units+)
                                                         (if (string/= "" actual-notes)
                                                             actual-notes
                                                             "none")
                                                         (get-clean-parameter +name-validity-date+)
                                                         (get-clean-parameter +name-expire-date+))
  ...
  (with-authentication
    (let ((all-ids (remove-if #'(lambda (a) (not (scan +pos-integer-re+ a)))
      ...
      (cond
        ...
        ((post-clean-parameter +op-submit-lend-to+)
         (lend-to (get-session-user-id) (post-clean-parameter +name-username-lending+)
                  (or (first all-ids) "")))
        ((post-clean-parameter +op-submit-massive-delete+)
         (massive-delete all-ids 'db:chemical-product
                         ...
                         #'manage-chem-prod))
        ((post-clean-parameter  +op-submit-change-shortage-threshold+)
         (manage-threshold  (post-clean-parameter +name-chem-id+)
                            (post-clean-parameter +name-shortage-threshold+)))
        (t
         (manage-chem-prod nil nil :data nil))))))
...
(define-lab-route import-chem-prod ("/import-chem-prod/" :method :post)
  (with-authentication
    (let ((errors          '())
          ...
          (row-number       0)
          (imported-prod-id '())
          (building-id (if (integer-positive-validate (post-clean-parameter +name-import-building-id+))
                           (post-clean-parameter +name-import-building-id+)
                           +db-invalid-id+))
          (data-table  '()))
